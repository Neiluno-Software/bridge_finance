...
export abstract class SwapRouter {
  ...
  private static encodeV2Swap(
    ...
    const deadline = BigInt(Math.floor(new Date().getTime() / 1000) + 300)
    ...
    if (trade.tradeType === TradeType.EXACT_INPUT) {
      ...
      const exactInputParams = [performAggregatedSlippageCheck ? 0n : amountOut, path, recipient, deadline] as const
  ...
  public static swapCallParameters(trades: AnyTradeType, options: SwapOptions): MethodParameters {
    ...
    return {
      calldata: calldatas[0], //MulticallExtended.encodeMulticall(calldatas, options.deadlineOrPreviousBlockhash),
