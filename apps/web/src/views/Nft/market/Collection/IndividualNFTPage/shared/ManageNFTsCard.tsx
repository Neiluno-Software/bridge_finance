        ...
        {lowestPrice && (
          <>
            ...
            <Flex justifySelf="flex-end" width="max-content" alignItems="center">
              <EthChainIcon width="16px" height="16px" mr="4px" />
...
const ManageNFTsCard: React.FC<React.PropsWithChildren<ManageNftsCardProps>> = ({
  ...
  const {
    ...
    isLoading,
    refresh,
  } = useCollectionsNftsForAddress(account, { [collection.address]: collection })
  ...
  const userHasNoNfts = !isLoading && nftsInWallet.length === 0 && nftsForSale.length === 0
  const totalNfts = nftsInWallet.length + nftsForSale.length
