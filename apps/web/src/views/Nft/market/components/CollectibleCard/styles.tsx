...
export const ETHAmountLabel: React.FC<React.PropsWithChildren<ETHAmountLabelProps>> = ({ amount, ...props }) => (
  <Flex alignItems="center" {...props}>
    <EthChainIcon width="16px" mx="4px" />
    <Text fontWeight="600">
...
interface CostLabelProps extends FlexProps {
  cost: number
  ethBusdPrice: BigNumber
}

export const CostLabel: React.FC<React.PropsWithChildren<CostLabelProps>> = ({ cost, ethBusdPrice, ...props }) => {
  const priceInUsd = ethBusdPrice.multipliedBy(cost).toNumber()
...
export const LowestPriceMetaRow = ({ lowestPrice, isFetching, ethBusdPrice }: LowestPriceMetaRowProps) => {
  const { t } = useTranslation()
  ...
  return (
    <MetaRow title={t('Lowest price')}>
      {isFetching ? (
        ...
        <CostLabel cost={lowestPrice} ethBusdPrice={ethBusdPrice} />
      )}
    </MetaRow>
  )
}
