...
import { Box, Button, ErrorIcon, EthChainIcon, Flex, Grid, Skeleton, Text, useTooltip } from '@pancakeswap/uikit'
...
import { EthAmountCell, FeeAmountCell, GreyedOutContainer, RightAlignedInput } from './styles'
...
const SetPriceStage: React.FC<React.PropsWithChildren<SetPriceStageProps>> = ({
  ...
  const priceInUsd = ethPrice.multipliedBy(priceAsFloat).toNumber()
    ...
    <>
      ...
      <GreyedOutContainer>
        ...
        <Flex>
          <Flex flex="1" alignItems="center">
            <EthChainIcon width={24} height={24} mr="4px" />
            <Text bold>WETH</Text>
          </Flex>
          <Flex flex="1">
            <RightAlignedInput
        ...
        <Flex justifyContent="space-between" alignItems="center" mt="16px">
          ...
          {Number.isFinite(creatorFeeAsNumber) && Number.isFinite(tradingFeeAsNumber) ? (
            <FeeAmountCell ethAmount={priceAsFloat} creatorFee={creatorFeeAsNumber} tradingFee={tradingFeeAsNumber} />
          ) : (
            <Skeleton width={40} />
          )}
        </Flex>
        {lowestPrice ? (
          <Flex justifyContent="space-between" alignItems="center" mt="16px">
            ...
            <EthAmountCell ethAmount={lowestPrice} />
          </Flex>
        ):null}
      </GreyedOutContainer>
      <Grid gridTemplateColumns="32px 1fr" p="16px" maxWidth="360px">
        ...
        <Box>
          ...
          <Text small color="textSubtle">
            {t('Sales are in WETH. You can swap WETH to ETH 1:1 for free with Orb3.')}
          </Text>
        </Box>
      </Grid>
      <Divider />
