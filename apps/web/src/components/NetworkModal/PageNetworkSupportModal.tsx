...
export function PageNetworkSupportModal() {
  ...
  const lastValidPath = historyManager?.history?.find((h) => ['/', '/bridge', '/history'].includes(h))
  ...
  return (
    <Modal title={title || t('Check your network')} hideCloseButton headerBackground="gradientCardHeader">
      <Grid style={{ gap: '16px' }} maxWidth="360px">
        <Text bold>{t('It’s a Orb3 only feature')}</Text>
        ...
        <Text small>
          {t('This features are currently available only on Orb3 Chain! Come over and join the community in the fun!')}
        </Text>
        {canSwitch ? (
          <Button
            variant={foundChain && lastValidPath ? 'secondary' : 'primary'}
            isLoading={isLoading}
            onClick={() => (isWrongNetwork ? switchNetworkLocal(ChainId.ORB3) : switchNetworkAsync(ChainId.ORB3))}
          >
            {t('Switch to %chain%', { chain: 'Orb3' })}
          </Button>
        ) : (
