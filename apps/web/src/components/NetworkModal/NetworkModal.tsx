...
import { SUPPORT_ONLY_ORB3 } from 'config/constants/supportChains'
...
export const NetworkModal = ({ pageSupportedChains = SUPPORT_ONLY_ORB3 }: { pageSupportedChains?: number[] }) => {
  ...
  const isOrb3OnlyPage = useMemo(() => {
    return pageSupportedChains?.length === 1 && pageSupportedChains[0] === ChainId.ORB3
  }, [pageSupportedChains])
  ...
  if (isPageNotSupported && isOrb3OnlyPage) {
    return (
